    ...
    return (
      <div className="App">
        ...
        <form onSubmit={this.addTodo}>
          <input data-test-id="add-todo" type="text" placeholder="Add a todo" name="todo" value={todo} onChange={this.handleChange} />
          <button type="submit">Add Todo</button>
        </form>
        <ul data-test-id="todos">
          {todos.map((todo) => (
            <Todo {...todo} key={todo.id} onComplete={() => { this.completeTodo(todo.id) }} />
          ))}
        </ul>
      </div>
    )
